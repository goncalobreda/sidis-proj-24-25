...
public class RabbitMQConfig {
    ...
    public static final String AUTHOR_EVENT_QUEUE = "author-event-queue";
    ...
    @Bean
    public Queue authorEventQueue() {
        return new Queue(AUTHOR_EVENT_QUEUE, true);
    }

    @Bean
    public TopicExchange authorCommandExchange() {
        return new TopicExchange(AUTHOR_COMMAND_EXCHANGE);
    }

    @Bean
    public Binding authorEventQueueBinding(Queue authorEventQueue, TopicExchange authorCommandExchange) {
        return BindingBuilder.bind(authorEventQueue).to(authorCommandExchange).with(AUTHOR_ROUTING_KEY);
    }
